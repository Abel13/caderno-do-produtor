...
export const foliarFertilizationComponentSchema = z.object({
  productName: z.string().trim().min(1, "Informe o produto.").max(120, "Use até 120 caracteres."),
  doseValue: nonNegative.refine((value) => value !== null, "Informe a dose ou concentração."),
  doseUnit: z.string().trim().min(1, "Informe a unidade.").max(40, "Use até 40 caracteres."),
  totalQuantity: nonNegative,
  notes: optionalText,
});

export const foliarFertilizationSchema = z
  .object({
    propertyId: z.string().uuid("Selecione a propriedade."),
    foliarFertilizationId: z.preprocess((value) => (value === "" || value == null ? undefined : String(value)), z.string().uuid().optional()),
    plotId: z.string().uuid("Selecione o talhão."),
    plantingId: optionalUuid,
    seasonId: optionalUuid,
    appliedOn: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Informe a data de aplicação."),
    purpose: z.string().trim().min(1, "Informe a finalidade.").max(120, "Use até 120 caracteres."),
    sprayVolumeLHa: nonNegative,
    temperatureC: optionalDecimal.refine((value) => value === null || (value >= -10 && value <= 60), "Informe temperatura entre -10 e 60 °C."),
    humidityPct: percentage,
    windSpeedKmH: nonNegative,
    ...
    notes: optionalText,
    clientId: z.preprocess((value) => (value === "" || value == null ? null : String(value)), z.string().uuid().nullable().optional()),
    components: z.array(foliarFertilizationComponentSchema).min(1, "Informe ao menos um componente da mistura."),
  })
  .superRefine((value, ctx) => {
    const applied = new Date(`${value.appliedOn}T00:00:00.000Z`);
    const tomorrow = new Date();
...
export const foliarFertilizationFilterSchema = z.object({
  propertyId: z.string().uuid(),
  plotId: z.preprocess((value) => (value === "" || value == null ? undefined : String(value)), z.string().uuid().optional()),
  plantingId: z.preprocess((value) => (value === "" || value == null ? undefined : String(value)), z.string().uuid().optional()),
  seasonId: z.preprocess((value) => (value === "" || value == null ? undefined : String(value)), z.string().uuid().optional()),
  purpose: z.preprocess((value) => (value === "" || value == null ? undefined : String(value)), z.string().trim().max(120).optional()),
  productName: z.preprocess((value) => (value === "" || value == null ? undefined : String(value)), z.string().trim().max(120).optional()),
  from: z.preprocess((value) => (value === "" || value == null ? undefined : String(value)), z.string().max(10).optional()),
  to: z.preprocess((value) => (value === "" || value == null ? undefined : String(value)), z.string().max(10).optional()),
  showDeleted: z.preprocess((value) => value === "1" || value === true, z.boolean()),
});
...
export type FoliarFertilizationInput = z.infer<typeof foliarFertilizationSchema>;
export type FoliarFertilizationFilterInput = z.infer<typeof foliarFertilizationFilterSchema>;
