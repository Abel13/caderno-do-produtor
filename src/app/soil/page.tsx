...
import { formatSoilDecimal, summarizeFoliarFertilizations, summarizeSoilAnalyses, summarizeSoilCorrections, summarizeSoilFertilizations } from "@/modules/soil-nutrition/domain/rules";
...
export default async function SoilPage() {
  ...
  const [analyses, corrections, fertilizations, foliarFertilizations] = await Promise.all([
    ...
    repository.listFoliarFertilizations({ propertyId: property.id, showDeleted: false }),
  ]);
  ...
  const foliarSummary = summarizeFoliarFertilizations(foliarFertilizations);
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <section className="mt-6 rounded-2xl bg-emerald-900 p-5 text-white">
          ...
          <p className="mt-2 max-w-2xl text-sm text-emerald-50">
            Preencha análises, correções, adubações via solo e adubações via folha como fichas do caderno, com histórico por talhão e safra.
          </p>
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <a href="/soil/foliar-fertilizations" className="inline-flex min-h-11 items-center rounded-xl bg-emerald-700 px-4 text-sm font-semibold text-white ring-1 ring-emerald-300">
              {canManage ? "Registrar adubação via folha" : "Consultar foliares"}
            </a>
          </div>
        </section>
        ...
        <section className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <Metric label="Adubações via folha" value={String(foliarSummary.fertilizationsCount)} helper={`${formatSoilDecimal(foliarSummary.totalSprayVolumeLHa)} L/ha de calda registrados.`} />
        </section>
